...
## Run Fully Local (Gemma 4 — no API keys)

OpenClicky can run completely on-device with **no API keys and no `.env`**. Voice in, reasoning, and voice out all happen locally, and nothing leaves your machine:

```
Apple on-device Speech (STT)  →  Gemma 4 12B via Ollama (LLM)  →  macOS system voice (TTS)
```

There is no cloud call in this path — no Anthropic, OpenAI, or ElevenLabs key required. All data stays local.

### 1. Install Ollama and pull the model

Use the official Ollama app (the Homebrew formula ships without the model runner):

```sh
brew install --cask ollama
open -a Ollama   # starts the local server on http://127.0.0.1:11434
ollama pull hf.co/unsloth/gemma-4-12b-it-GGUF:UD-Q4_K_XL
```

The 12B QAT model needs ~8 GB of RAM and runs ~24 tok/s on an Apple Silicon Mac (M-series). On a 16 GB machine prefer the smaller `gemma-4-12b` is still fine; lighter `E4B` variants exist if you are memory-constrained. Point OpenClicky at a non-default server with the `OLLAMA_BASE_URL` environment variable if needed.

### 2. Enable macOS Dictation

Apple's on-device speech recognizer requires Dictation to be on:

- System Settings → Keyboard → **Dictation** → On (downloads the on-device language model).

### 3. Select the local options in OpenClicky

After launching the app:

- Settings → Voice → **Response voice model** → `Gemma 4 12B (Local)`
- Settings → Voice → **Listening / transcription** → `Apple` (on-device)
- Settings → Voice → **Playback engine** → `macOS Voice (Local)`

Grant Accessibility, Microphone, and Speech Recognition when prompted. Then push-to-talk (default `Control + Option + K`) and ask a question — the answer is generated by the local model and spoken by the system voice, with no keys configured anywhere.

> Tip: for a nicer voice, download an enhanced/Siri voice under System Settings → Accessibility → Spoken Content → System Voice (still on-device).

Agent Mode and screen-pointing/vision are not part of the local path yet; those still use the configured cloud providers when enabled.
